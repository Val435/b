...
export const fetchFullRecommendation: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (!user) {
      ...
      return; // ⬅️ corta la ejecución, sin devolver Response
    }

    // 2️⃣ Obtener la recomendación (filtra por journeyId si vino)
    ...
    res.status(200).json({ success: true, data: response }); // ⬅️ sin `return`
  } catch (error) {
    next(error);
  }
};